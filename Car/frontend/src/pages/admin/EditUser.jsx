...
import { ArrowLeft, Save, User, Mail, Phone, Shield, ShieldAlert, X, CheckCircle, Smartphone } from 'lucide-react';
...
export default function EditUser() {
  ...
  const fetchUser = async () => {
    try {
      setLoading(true);
      const res = await api.get(`/users/admin-users/${id}/`, { useAdminToken: true });
      const user = res.data;
      setFormData({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.patch(`/users/admin-users/${id}/`, payload, { useAdminToken: true });
      alert(t('admin.client_relations.success_update'));
      navigate('/admin/users');
    } catch (err) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-6 mb-12">
           ...
           <div>
              <h1 className="text-4xl font-black text-white italic tracking-tighter uppercase mb-1">
                {t('admin.client_relations.edit_user')}
              </h1>
              <p className="text-silver-500 text-xs font-black uppercase tracking-[0.2em]">{t('admin.client_relations.uid')} {id}</p>
           </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
           ...
           <div className="lg:col-span-2 space-y-8">
              <form onSubmit={handleSubmit} className="glass-panel p-10 rounded-[32px] border border-white/5 shadow-2xl relative overflow-hidden">
                 <div className="absolute top-0 right-0 p-10 opacity-[0.02] pointer-events-none">
                    <User className="w-64 h-64 text-gold-500" />
                 </div>
                 ...
                 <div className="space-y-10 relative">
                    ...
                    <div className="space-y-3">
                       <label className="text-[10px] font-black text-silver-500 uppercase tracking-widest flex items-center gap-2">
                          <ShieldAlert className="w-3 h-3 text-gold-500" /> {t('admin.client_relations.password')}
                       </label>
                       <input 
                         type="password"
                         className="luxury-input w-full py-4 px-6 text-base"
                         ...
                         placeholder={t('admin.client_relations.password_placeholder')}
                       />
                    </div>
                    ...
                    <div className="flex gap-4 pt-6">
                       ...
                       >
                         ...
                         {t('admin.client_relations.save_user')}
                       </button>
                    </div>
                 </div>
              </form>
           ...
           <div className="space-y-8">
              ...
              <div className="glass-panel p-8 rounded-[32px] border border-white/5 shadow-xl">
                 <h4 className="text-[10px] font-black text-silver-500 uppercase tracking-widest mb-4">{t('admin.client_relations.security_logs')}</h4>
                 <div className="space-y-4">
                    <div className="flex items-start gap-3">
                       <CheckCircle className="w-3.5 h-3.5 text-green-500 shrink-0 mt-0.5" />
                       <p className="text-[11px] text-silver-400 font-medium">{t('admin.client_relations.sync_status')}</p>
                    </div>
                 </div>
              </div>
           </div>
